...
class MOQLearning(MOPolicy, MOAgent):
    ...
    def __init__(
        ...
        scalarization: float = weighted_sum,
    ...
    def scalarized_q_values(self, obs, w: np.ndarray) -> np.ndarray:
        ...
        return np.array(
            [
                self.scalarization(state_action_value, w)
    ...
    def _gpi_pd_priority(
        ...
        priority = (
            ...
            + (1 - terminal)
            * self.gamma
            * self.parent.max_scalar_q_value(next_obs, weights)
    ...
    def update(self):
        ...
        if self.dyna:
            if self.gpi_pd:
                priority = self._gpi_pd_priority(
                    obs,
                    self.action,
                    self.reward,
                    next_obs,
            ...
            self.model.update(
                obs, self.action, self.reward, next_obs, self.terminated, priority
            )
            for _ in range(self.dyna_updates):
                ...
                if self.gpi_pd:
                    priority = self._gpi_pd_priority(
                        s, a, r, next_s, terminal, self.weights
                    )
            ...
            wandb.log(
                {
                    ...
                    f"losses{self.idstr}/scalarized_td_error": self.scalarization(
                        td_error, self.weights
                    ),
...
"""    def save_hyperparams_json(self, filename):
        Saves hyperparameters to a JSON file.

        Args:
            hyperparams (dict): A dictionary containing the hyperparameters to save.
            filename (str): The filename to save the data to (e.g., 'hyperparams.json').
        
        # Convert NumPy arrays and function names appropriately for JSON
        try:
            data = {
                "id": self.id,
                # "scalarization":self.scalarization,
