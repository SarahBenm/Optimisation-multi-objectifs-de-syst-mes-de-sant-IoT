...
class BasePCNModel(nn.Module, ABC):
    ...
    def __init__(
        ...
        self.scaling_factor = nn.Parameter(
            th.tensor(scaling_factor).float(), requires_grad=False
        )
...
class DiscreteActionsDefaultModel(BasePCNModel):
    ...
    def __init__(
        ...
        self.s_emb = nn.Sequential(
            nn.Linear(self.state_dim, self.hidden_dim), nn.Sigmoid()
        )
        self.c_emb = nn.Sequential(
            nn.Linear(self.reward_dim + 1, self.hidden_dim), nn.Sigmoid()
        )
...
class PCN(MOAgent, MOPolicy):
    ...
    def __init__(
        ...
        self.continuous_action = (
            True if type(self.env.action_space) is gym.spaces.Box else False
        )
        self.noise = noise
    ...
    def update(self):
        ...
        s_i = self.np_random.choice(
            np.arange(len(self.experience_replay)), size=self.batch_size, replace=True
        )
        for i in s_i:
            ...
            s_t, a_t, r_t, h_t = (
                ep[t].observation,
                ep[t].action,
                np.float32(ep[t].reward),
                np.float32(len(ep) - t),
            )
            batch.append((s_t, a_t, r_t, h_t))

        obs, actions, desired_return, desired_horizon = zip(*batch)
        # print("obs, actions, desired_return, desired_horizon",obs, actions, desired_return, desired_horizon)
        ...
        if self.continuous_action:
            ...
            actions = F.one_hot(
                th.tensor(actions).long().to(self.device), len(prediction[0])
            )
            # cross-entropy loss
    ...
    def _add_episode(
        self, transitions: List[Transition], max_size: int, step: int
    ) -> None:
        # compute return
    ...
    def _nlargest(self, n, threshold=0.2):
        ...
        for i in range(len(l2)):
            self.experience_replay[i] = (
                l2[i],
                self.experience_replay[i][1],
                self.experience_replay[i][2],
            )
    ...
    def _act(
        self, obs: np.ndarray, desired_return, desired_horizon, eval_mode=False
    ) -> int:
        ...
        if self.continuous_action:
            ...
            if eval_mode:
                ...
                action = self.np_random.choice(
                    np.arange(len(log_probs)), p=np.exp(log_probs)
                )
            return action

    def _run_episode(
        self, env, desired_return, desired_horizon, max_return, eval_mode=False
    ):
        transitions = []
        obs, _ = env.reset()
        done = False
        while not done:
            ...
            desired_return = np.clip(
                desired_return - reward, None, max_return, dtype=np.float32
            )
            # clip desired horizon to avoid negative horizons
            desired_horizon = np.float32(max(desired_horizon - 1, 1.0))
        return transitions

    def set_desired_return_and_horizon(
        self, desired_return: np.ndarray, desired_horizon: int
    ):
    ...
    def evaluate(self, env, max_return, n=10):
        ...
        e_returns = []
        for i in range(n):
            transitions = self._run_episode(
                env, returns[i], np.float32(horizons[i] - 2), max_return, eval_mode=True
            )
            # compute return
            for i in reversed(range(len(transitions) - 1)):
                transitions[i].reward += self.gamma * transitions[i + 1].reward
    ...
    def train(
        ...
        max_return = (
            ...
            else np.full(self.reward_dim, 100.0, dtype=np.float32)
        )
        if self.log:
        ...
        for _ in range(num_er_episodes):
            ...
            while not done:
                ...
                transitions.append(
                    Transition(
                        obs, action, np.float32(reward).copy(), n_obs, terminated
                    )
                )
                done = terminated or truncated
                obs = n_obs
            ...
            self._add_episode(
                transitions, max_size=max_buffer_size, step=self.global_step
            )
        ...
        while self.global_step < total_timesteps:
            ...
            leaves_r = np.array(
                [
                    e[2][0].reward
                    for e in self.experience_replay[len(self.experience_replay) // 2 :]
                ]
            )
                ...
                wandb.log(
                    {
                        ...
                        "train/mean_horizon_distance": np.linalg.norm(
                            np.mean(horizons) - desired_horizon
                        ),
                        "global_step": self.global_step,
                    },
                )
