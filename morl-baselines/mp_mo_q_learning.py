...
class MPMOQLearning(MOAgent):
    ...
    def __init__(
        ...
        Args:
            ...
            trained_folder: folder where model is stored after training
        """
        ...
        self.linear_support = LinearSupport(
            num_objectives=self.reward_dim, epsilon=epsilon_ols
        )
        ...
        # init saving folder for trained model :
    ...
    def _gpi_action(self, state: np.ndarray, w: np.ndarray) -> int:
        ...
        """
        q_vals = np.stack(
            [policy.scalarized_q_values(state, w) for policy in self.policies]
        )
    ...
    def train(
        ...
        for iter in range(num_iterations):
            if (
                self.weight_selection_algo == "ols"
                or self.weight_selection_algo == "gpi-ls"
            ):
                ...
                if w is None:
                    print(
                        "OLS has no more corner weights to try. Using a random weight instead."
                    )
            ...
            value = policy_evaluation_mo(
                agent=new_agent, env=eval_env, w=w, rep=num_eval_episodes_for_front
            )[3]
    ...
    """def save_policies(self):
        Saves Q-tables of all policies to individual files within numbered folders.

        This function iterates through the `self.policies` list, creating a new folder
        inside `./trained/` for each policy (e.g., `./trained/policy_0`, `./trained/policy_1`).
        Within each folder, it calls `save_q_table` to save the policy's Q-table as a NumPy array.

        Raises:
            OSError: If an error occurs while creating directories or saving files.
        

        i = 0
        for policy in self.policies:
            # Create a numbered folder for each policy within ./trained/
            folder_path = path_join_folder_policies(self.trained_folder, i)
            try:
                os.makedirs(
            ...
            # Save the Q-table using save_q_table
            save_q_table(policy.q_table, directory=folder_path)
            json_path = os.path.join(folder_path, "hyper_par.json")
            policy.save_hyperparams_json(json_path)
            i += 1

    def load_policies(self, dir_path):
        try:
            ...
            for entry in entries:
                ...
                json_path = os.path.join(full_path, "hyper_par.json")
                hyperparams = load_hyperparams_json(json_path)
                if os.path.isdir(full_path):
                    ...
                    policy.q_table = load_q_table(full_path)
...
def save_q_table(data, directory="."):
    Saves a Q-table as a NumPy array to the specified directory.

    Args:
        data: The Q-table data to be saved (assumed to be a NumPy array).
        directory: The directory path where the file should be saved (default: ".")
...
def load_q_table(directory="."):
    Loads NumPy arrays from separate files into a dictionary.

    Args:
        directory: The directory containing the saved files (default: current working directory).

    Returns:
        A dictionary where keys are reconstructed from filenames (based on assumptions)
        and values are loaded NumPy arrays.
    
    data = {}
    for filename in os.listdir(directory):
        if filename.endswith(".npy"):
            key_pattern = r"(\d+)"  # Matches characters enclosed in parentheses (adjust for your format)
            match = re.search(key_pattern, filename)
            if match:
                key_str = match.group(1)  # Extract the captured group (key)
                try:
                    # Assuming integer keys, convert to integer and create a tuple
                    key = int(key_str)
                    key = (key,)
                except ValueError:
                    # Handle non-numeric keys (modify for your key type)
                    print(
                        f"Warning: Could not convert key '{key_str}' from '{filename}': Not a number. Skipping."
                    )
                    continue

                filepath = os.path.join(directory, filename)
                data[key] = np.load(filepath)
    print("Dictionary loaded from files (with fixed key conversion and tuples).")
    return data


def load_hyperparams_json(filename):
    Loads hyperparameters from a JSON file.

    Args:
        filename (str): The filename of the JSON file containing the hyperparameters.

    Returns:
        dict: A dictionary containing the loaded hyperparameters, or None if an error occurs.
    """
...
"""
    try:
        ...
        print(f"Error loading hyperparameters from JSON: {e}")
        return None  # Or raise an exception if loading is critical
...
def path_join_folder_and_int_list(folder_path, int_list) -> str:
    ...
    return joined_path + ".csv"


def path_join_folder_policies(folder_path, i: int) -> str:
    ...
    joined_path = folder_path + "/" + f"policy_{i}"
    return joined_path
"""
